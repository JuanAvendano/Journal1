...
def Statistics(k,save_info,start,end):
    ...
    pathresults= path+'\\Error Results'
    ...
    # 2.2 Create list with reference measurements, measurements made using BML and corresponding error
    # # ===============================================================================================================
    ...
    for n in range(start, end+1):
        ...
        # Path for the coord. of the points
        pathsBML = path + '\BME\Crack ' + str(n) + '\\k=' + str(method_threshold) + '\\'
        ...
        points_filename = os.path.join(pathsBML, 'Points_for_results_BME.txt')
    ...
    for i in range(len(full_list)):
        try:
            ...
            BML_measure=full_list[i][8]
            error = round((BML_measure-ref_measure)/ref_measure*100, 2) # round the error value to 2 decimal places
        except ValueError:
            error = np.nan
    ...
    if save_info == True:
        ...
        results_filename = 'BME_Full_List_Results MAD k=' + str(method_threshold)+'.txt'
        # Columns names
        ...
        with open(pathresults + '//' + results_filename, "w") as output:
